 ...
 */
 ...
 __global__ void histogramKernel2D(const int nop, const histogramTypeIn *d1, const histogramTypeIn *d2, 
                                    const histogramTypeIn *q,
                                    particleHistogramCUDA2D *histogramCUDAPtr)
{
    ...
    constexpr int tile = config::PARTICLE_HISTOGRAM2D_TILE;
    constexpr auto tileSize = tile * tile;

    histogramTypeIn data[2];
    int dim[2];
    int dimSize[2] = {tile, tile};
    histogramTypeOut qAbs;
 ...
 * @details this kernel is launched once for each histogram bin for all 3 histograms
 */
__global__ void resetBin(particleHistogramCUDA2D* histogramCUDAPtr){
    int idx = threadIdx.x + blockIdx.x * blockDim.x;
    int gridSize = gridDim.x * blockDim.x;
...
__host__ void particleHistogram2D::init(histogramTypeIn* xArrayDevicePtr, histogramTypeIn* yArrayDevicePtr, histogramTypeIn* qArrayDevicePtr, const int pclNum, const int species, cudaStream_t stream){

    getRange(xArrayDevicePtr, yArrayDevicePtr, pclNum, species, stream);
    histogramHostPtr->setHistogram(minArray, maxArray, binThisDim);
    cudaErrChk(cudaMemcpyAsync(histogramCUDAPtr, histogramHostPtr, sizeof(particleHistogramCUDA2D), cudaMemcpyHostToDevice, stream));
    ...
    histogramKernel2D<<<getGridSize(pclNum / 128, 512), 512, sharedMemSize, stream>>>
 ...
 * @brief Synchronous function to get the min and max for 3 dimensions, result is stored in minArray and maxArray
 */
__host__ int particleHistogram2D::getRange(histogramTypeIn* xArrayDevicePtr, histogramTypeIn* yArrayDevicePtr, const int pclNum, const int species, cudaStream_t stream){
