...
public:
    ...
    __host__ void preProcessDataGMM(const T* meanArray, const T* maxVelocityArray){
        ...
        if constexpr(config::NORMALIZE_DATA_FOR_GMM) normalizePoints(maxVelocityArray);
    }
    ...
    __host__ int initGMM(){
        ...
        while(step < paramHostPtr->maxIteration){
            ...
            if constexpr (config::PRUNE_COMPONENTS_GMM){
    ...
    __host__ void postProcessDataGMM(const T* maxVelocityArray){
        ...
        if constexpr(config::NORMALIZE_DATA_FOR_GMM) normalizeDataBack(maxVelocityArray);        
    }
...
private:
    ...
    T sumLogLikelihood(){ // its sync now, but can be async with the M step
        ...
        reduction::reduceSumPreProcess<T, blockSize, reduction::PreProcessType::none, void, U, true> // weighted sum
        ...
        reduction::reduceSumWarp<T><<<1, WARP_SIZE, 0, GMMStream>>>(reductionTempArrayCUDA, logLikelihoodCUDA, blockNum);
    ...
    void calcPosterior(){ // the Big Gamma, it can be direct sum then log
        ...
        for(int component = 0; component < paramHostPtr->numComponents; component++){
            ...
            reduction::reduceSumPreProcess<T, blockSize, reduction::PreProcessType::minusConstThenEXP, T, U, true>
                <<<blockNum, blockSize, blockSize*sizeof(T), GMMStream>>>
                (posteriorComponent, reductionTempArrayCUDA, dataHostPtr->getNumData(), maxValueArray + component, dataHostPtr->getWeight());
            
            reduction::reduceSumWarpPostProcess<T, reduction::PostProcessType::logAdd, T>
                <<<1, WARP_SIZE, 0, GMMStream>>>
    ...
    void updateMean(){
        ...
        for(int component = 0; component < paramHostPtr->numComponents; component++){
            ...
            for(int dim = 0; dim < dataHostPtr->getDim(); dim++){
                ...
                reduction::reduceSumPreProcess<T, blockSize, reduction::PreProcessType::multiplyEXP, T, U, true>
                    <<<blockNum, blockSize, blockSize*sizeof(T), GMMStream>>>
                ...
                reduction::reduceSumWarpPostProcess<T, reduction::PostProcessType::divideEXP, T>
                    <<<1, WARP_SIZE, 0, GMMStream>>>
    ...
    void updateCoVarianceAndDecomposition(){
        ...
        for(int component = 0; component < paramHostPtr->numComponents; component++){
            ...
            for(int element = 0; element < dataDim * dataDim; element++){
                reduction::reduceSumPreProcess<T, blockSize, reduction::PreProcessType::none, void, U, true>
                    <<<blockNum, blockSize, blockSize*sizeof(T), GMMStream>>>
        ...
        if constexpr(config::CHECK_COVMATRIX_GMM){
            kernel::checkAdjustCoVarianceKernel<T, dataDim>
        ...
        kernel::decomposeCoVarianceKernel<T, dataDim>
    ...
    bool pruneOneComponent(const int step){
        ...
        *flagResetMean = false;
        ...
        // launch kernel
        kernel::pruneOneComponentKernel
        <<<1, paramHostPtr->numComponents, paramHostPtr->numComponents * (sizeof(T) + sizeof(bool)) + sizeof(bool), GMMStream>>>
        (weightCUDA, config::PRUNE_THRESHOLD_GMM, paramHostPtr->numComponents, flagActiveComponentsCUDA);
        ...
        // launch kernel
        kernel::checkMeanValueComponents<T, dataDim>
