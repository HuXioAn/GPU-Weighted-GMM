 ...
 */
template <typename T, int dataDim, typename U>
__global__ void calcLogLikelihoodForPointsKernel(const weightedGMM::GMMDataMultiDim<T, dataDim, U>* dataCUDAPtr, const T* meanVector, const T* coVarianceDecomp, 
 ...
 */
template <typename T, int dataDim, typename U>
__global__ void calcLogLikelihoodPxAndposteriorKernel(const weightedGMM::GMMDataMultiDim<T, dataDim, U>* dataCUDAPtr, const T* logWeightVector, const T* logLikelihoodForPoints, 
 ...
 */
template <typename T, int dataDim, typename U>
__global__ void updateCoVarianceKernel(const weightedGMM::GMMDataMultiDim<T, dataDim, U>* dataCUDAPtr, const T* logPosterior_nk, 
 ...
 */
 ...
 __global__ void checkAdjustCoVarianceKernel(T* coVariance, const int numComponents, const bool* flagActiveComponents){
     ...
     for(int i = 0; i<dataDim; i++ ){
         ...
         for(int j = 0; j<dataDim; j++){
             if(std::isnan(coVarianceComponent[ i*dataDim + j])){
                 coVarianceComponent[ i*dataDim + j] = config::TOL_COVMATRIX_GMM;
             }
         }
 
         if(coVarianceComponent[ i*dataDim + i] < config::EPS_COVMATRIX_GMM ){
             coVarianceComponent[ i*dataDim + i] = config::EPS_COVMATRIX_GMM;
         }
     }
     ...
     if constexpr(dataDim == 2)
     {
         ...
         if(coVarianceComponent[0]*coVarianceComponent[3] - coVarianceComponent[2]*coVarianceComponent[2] - config::TOL_COVMATRIX_GMM <=0){
             const T k = coVarianceComponent[3] / coVarianceComponent[0]; 
             coVarianceComponent[0] = sqrt( (coVarianceComponent[2]*coVarianceComponent[2] + config::TOL_COVMATRIX_GMM) / k  ) + sqrt(config::TOL_COVMATRIX_GMM);
     ...
     else if constexpr(dataDim == 3)
     {
         ...
         for(int j = 0; j < 50; j++)
         {  
            if( determinant - config::TOL_COVMATRIX_GMM > 0 )break;
            coVarianceComponent[0] += 20*config::TOL_COVMATRIX_GMM;
...
__global__ void normalizePointsKernel(weightedGMM::GMMDataMultiDim<T, dataDim, U>* dataCUDAPtr, const T* meanDataInitCUDA, const T* rescaleFactor){
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    auto numData = dataCUDAPtr->getNumData();
    if(idx >= numData)return;
