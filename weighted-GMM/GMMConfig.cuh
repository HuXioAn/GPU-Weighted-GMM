...
inline constexpr bool NORMALIZE_DATA_FOR_GMM = false;    // normalize data before GMM such that velocities are in range -1;1 --> the original velocity domain is assumed to be symmetric wrt 0
...
inline constexpr bool PRUNE_COMPONENTS_GMM = false; // remove GMM components with weight < PRUNE_THRESHOLD_GMM --> remove one componet at a time
